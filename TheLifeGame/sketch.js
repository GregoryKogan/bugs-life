...
function SetUpUI(){
  ...
  FoodSpawningTypeCheckBox.style('font-size', '15px');
...
function SwitchStats(){
  ...
  else if (ShowUI == false && ShowChangeMenu == false){
    ShowStatistics = !ShowStatistics;
...
function SwitchInterface(){
  if (!fullscreen()) fullscreen(true);
  if (ShowUI == false && ShowStatistics == false && ShowChangeMenu == false){
    ShowUI = !ShowUI;
...
function draw() {
  ...
  if (Device != "Laptop") text("v.1.2", windowWidth - 40, windowHeight - 20);
